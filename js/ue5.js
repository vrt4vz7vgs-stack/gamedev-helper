...
const UE5Lib = (function () {
  ...
  const terrainHeader = `// ProceduralTerrain.h — runtime-generated terrain using Perlin noise.
...
float AProceduralTerrain::GetHeight(float WorldX, float WorldY) const
{
  ...
  const foliageHeader = `// FoliageSpawner.h — scatters trees/rocks on the terrain using
...
UCLASS()
class TERRAINGEN_API AFoliageSpawner : public AActor
{
	GENERATED_BODY()
...
private:
  ...
  const foliageCpp = `// FoliageSpawner.cpp
...
void AFoliageSpawner::Scatter()
{
	...
	for (float X = -Half; X <= Half; X += Spacing)
	{
		for (float Y = -Half; Y <= Half; Y += Spacing)
		{
			...
			const float Noise = FMath::PerlinNoise2D(FVector2D(WorldLocation.X, WorldLocation.Y) * 0.004f + FVector2D(1337.0f * 7.31f, 1337.0f * 13.17f));
...
and lerp them the same way to make color match elevation exactly.`;
  ...
  const ue5Terrain = {
    ...
    files: [
      { filename: "ProceduralTerrain.h", lang: "cpp", code: terrainHeader },
      { filename: "ProceduralTerrain.cpp", lang: "cpp", code: terrainCpp },
      { filename: "FoliageSpawner.h", lang: "cpp", code: foliageHeader },
      { filename: "FoliageSpawner.cpp", lang: "cpp", code: foliageCpp },
    ],
  };
  ...
  return { ue5Terrain, ue5IslandGuide };
})();
